...
public class Management extends DAO{
	...
	private void run() {
		while(true) {
			...
			if(menuNo == 1) {
				...
				System.out.println(member.toString()); // toString 하면 Member [id=1000, pw=0000, name=이]
			}
			else if(menuNo == 3) {
	...
	private int insertMember(Member member) {
		...
		try {
			...
			//String sql = "insert into member values (?,?,?)"; // 프로시저 말고 단일쿼리쓸 때 하던 방식
			String sql = "{call member_procedure(?,?,?)}"; // 프로시저 부르기
			sql = "{call out_proc(?,?)}";
			cstmt = conn.prepareCall(sql); // 펑션이나 프로시저 실행할 때 쓰는 변수
